  ...
  const fitnessReducer=(state=initialState,action)=>{
    switch (action.type)
    {
        ...
        case "DELETE_EXERCISE":
            {
            const temp=state.exercises.filter(item=>item._id!==action.payload)
            return {...state,exercises:temp,loading:false}
        }
        case "DELETE_FOOD":
            {
            const temp=state.foods.filter(item=>item._id!==action.payload)
            return {...state,foods:temp,loading:false}
        }
        case "DELETE_GOAL":
            {
            const temp=state.goals.filter(item=>item._id!==action.payload)
